package com.yr.common.utils.poi;
...
import com.yr.common.utils.file.FileTypeUtils;
import com.yr.common.utils.file.FileUploadUtils;
import com.yr.common.utils.file.ImageUtils;
import com.yr.common.utils.reflect.ReflectUtils;
...
public class ExcelUtil<T> {
     ...
     * @throws Exception
     */
    public List<T> importExcelNew(InputStream is) throws Exception {
        ExcelReader reader = cn.hutool.poi.excel.ExcelUtil.getReader(is);

        Map<Integer, XSSFPictureData> pictures = getPictures((XSSFSheet) reader.getSheet());
        Map<Integer, String> picUrlMap = new HashMap<>();

        for (Integer row : pictures.keySet()) {
            XSSFPictureData pictureData = pictures.get(row);
            byte[] data = pictureData.getData();
            // 上传文件路径
            String filePath = YrConfig.getUploadPath();
            // 上传并返回新文件名称
            String fileNam = DateUtils.datePath() + "/" + IdUtils.fastUUID() + "." + "png";
            File file = FileUploadUtils.getAbsoluteFile(FileUploadUtils.getDefaultBaseDir(), fileNam);
            FileOutputStream os = new FileOutputStream(file);
            os.write(data,0,data.length);
            os.flush();
            os.close();
            picUrlMap.put(row, FileUploadUtils.getPathFileName(FileUploadUtils.getDefaultBaseDir(), fileNam));
        }
        ...
        for (int i = 0; i < CollUtil.size(list); i++) {
            String picUrl = picUrlMap.get(0);
            T t = list.get(0);
            for (Field field : fields) {
                Excel annotation = field.getAnnotation(Excel.class);
                if (annotation != null && annotation.isPhoto()) {
                    ReflectUtil.setFieldValue(t, field, picUrl);
                }
            }
        }
        return list;
     ...
     * @return
     */
    public static Map<Integer, XSSFPictureData> getPictures(XSSFSheet xssfSheet) {
        Map<Integer, XSSFPictureData> map = new HashMap<>();
        ...
        for (XSSFShape shape : list) {
            ...
            map.put(xssfClientAnchor.getRow1(), pdata);
    ...
    public static String getAbsoluteFile(String filename) {
        String downloadPath = YrConfig.getDownloadPath() + filename;
