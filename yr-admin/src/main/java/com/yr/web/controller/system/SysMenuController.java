...
public class SysMenuController extends BaseController {
    ...
    public AjaxResult add(@Validated @RequestBody SysMenu menu) {
        ...
        if (0!=menu.getParentId()){//不等于0说明本级菜单不在根节点上
            SysMenu parentMenu = menuService.selectMenuById(menu.getParentId());
            if (parentMenu!=null){
                if (!parentMenu.getPlatform().equals(menu.getPlatform())){
                    return AjaxResult.error("新增菜单'" + menu.getMenuName() + "'失败，父子级菜单平台类型必须一致");
                }
            }
        }
